...
In this lab, I practiced destructuring an object, converting normal functions into arrow function expressions, using for..in  syntax with objects, using for..of syntax with arrays, creating a Node.js compatible code module using module.exports, importing a Node.js compatible code module using require, and using the spread operator.
...
const getCarMakeModelImplicit = (someCar) => `${someCar.make} ${someCar.model}`;
...
const getCarMakeModelExplicit = (someCar) => {
    return `${someCar.make} ${someCar.model}`;
};
...
const getCarMakeModelDestructure = (someCar) => {
    const {make, model} = someCar;
    return `${make} ${model}`;
}
